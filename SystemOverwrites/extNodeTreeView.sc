+ NodeTreeView {
	...
	makeWindow { |bounds, parent|
		...
		).font_(Font.sansSerif(16, true)).stringColor_(Color.grey(0.4)).align_(\center);
	...
	start { |interval = 0.5, actionIfFail|
		...
		pen = GUI.current.pen;
		font = Font.sansSerif(15);

		view.drawFunc = {
			var xtabs = 0, ytabs = 0, drawFunc, palette = QtGUI.palette;
			...
			drawFunc = {|group|
				...
				group.do({|node, index|
					if(node.value.isArray, {
						...
						pen.fillColor = palette.color(\window);
						pen.fillRect(rect);
						pen.strokeColor = palette.color(\windowText).alpha_(0.75);
						pen.strokeRect(rect);
						pen.color = palette.color(\windowText).alpha_(0.75);
						pen.stringInRect(
